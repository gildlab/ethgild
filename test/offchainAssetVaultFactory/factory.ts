...
describe("OffchainAssetVaultFactory Test", () =>{
    before(async () => {
        ...
        const Factory = await ethers.getContractFactory("OffchainAssetVaultFactory");
    ...
    it("Should createChild (createTypedChild)",async () => {
        ...
        let deployTrx = await  factory.connect(alice).createChildTyped(constructionConfig);
        const { sender, child } = await getEventArgs(deployTrx, "NewChild", factory);
