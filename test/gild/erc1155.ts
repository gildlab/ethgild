...
describe("erc1155 usage", async function () {
  ...
  it("should only send itself", async function () {
    ...
    const erc20BalanceAfter2 = await vault.connect(bob)["balanceOf(address)"](
      bob.address
    );
    assert(
    ...
    const erc1155BalanceAfter = await receipt.connect(alice)["balanceOf(address,uint256)"](
      alice.address,
      expected1155ID
    );
    assert(
    ...
    const erc1155BalanceAfter2 = await receipt.connect(bob)["balanceOf(address,uint256)"](
      bob.address,
      expected1155ID
    );
    assert(
