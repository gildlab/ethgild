...
contract Ounce is ERC1155, ERC20 {
    ...
    function unvault(uint256 _vaultId, uint256 _amount) external {
        // ERC1155 burn.
        _burn(msg.sender, _vaultId, _amount);
        // ERC20 burn.
        _burn(msg.sender, _amount.mul(FEE_NUMERATOR).div(FEE_DENOMINATOR));
        // ETH refund.
        (bool _refundSuccess, ) = msg.sender.call{value: _amount}("");
        require(_refundSuccess, "ETH_REFUND");
        emit Unvault(msg.sender, _vaultId, _amount);
    }

    receive() external payable {
        require(msg.value > 0, "ZERO_VALUE");
        nextVaultId = nextVaultId + 1;
        uint256 _amount = msg.value.mul(xauEthPrice());
        // ERC20 mint.
        _mint(msg.sender, _amount);
        // ERC1155 mint.
        _mint(msg.sender, nextVaultId, _amount, "");
        emit Vault(msg.sender, nextVaultId, _amount);
    }
}