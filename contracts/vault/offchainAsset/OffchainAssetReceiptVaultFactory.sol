...
import {
    ...
    ReceiptVaultConfig
} from "./OffchainAssetReceiptVault.sol";
...
contract OffchainAssetReceiptVaultFactory is ReceiptVaultFactory {
    ...
    function _createChild(bytes memory data_) internal virtual override returns (address) {
        OffchainAssetVaultConfig memory offchainAssetVaultConfig_ = abi.decode(data_, (OffchainAssetVaultConfig));

        address clone_ = Clones.clone(implementation);
        OffchainAssetReceiptVault(clone_).initialize(
            OffchainAssetReceiptVaultConfig(
                offchainAssetVaultConfig_.admin, _createReceipt(clone_, offchainAssetVaultConfig_.vaultConfig)
            )
        );
        return clone_;
    }
    ...
    function createChildTyped(OffchainAssetVaultConfig memory offchainAssetVaultConfig_)
        external
        returns (OffchainAssetReceiptVault)
    {
        return OffchainAssetReceiptVault(createChild(abi.encode(offchainAssetVaultConfig_)));
    }
}
